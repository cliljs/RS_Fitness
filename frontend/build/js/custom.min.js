...
function init_daterangepicker() {
    if (void 0 !== $.fn.daterangepicker) {
        ...
        $("#reportrange span").html(moment().subtract(29, "days").format("MMMM D, YYYY") + " - " + moment().format("MMMM D, YYYY")), $("#reportrange").daterangepicker(a, e), $("#reportrange").on("show.daterangepicker", function () {
            ...
            let g = $('#btnGenderSelector').html();

            let st = a.startDate.format("YYYY-MM-DD");
            let et = a.endDate.format("YYYY-MM-DD");
            init_echarts(st, et, g);
...
function init_calendar(objData = []) {
        ...
        var o, e = new Date,
            ...
            i = $("#calendar").fullCalendar({
                ...
                viewRender: function (view, element) {
                    ...
                    let cArray = view.title.split(" ");
                    let mName = cArray[0];
                    let yName = cArray[1];
                    let selectedMonth = months.indexOf(mName) + 1;
                    plotCalendar(selectedMonth,yName);
                },
                editable: 0,
                events: objData,
...
function init_echarts(st, et, gender) {
    let origDate = [];
    let varDates = [];
    let varGained = [];
    let varBurned = [];
    ...
    console.log(loop);
    console.log(gender);
    $('#btnGenderSelector').html(gender);
    while (loop <= end) {

        tempString = (loop.getMonth() + 1) + '/' + loop.getDate();
        varDates.push(tempString);
        origDate.push(new Date(loop).toISOString().split('T')[0]);
        varGained.push(0);
        varBurned.push(0);
    ...
    fireAjax('UsersController.php?action=get_admin_criteria&d1=' + st + '&d2=' + et + '&gender=' + gender, '', false).then(function (data) {
        console.log(data);
        let objData = $.parseJSON(data.trim()).data;
        let objBurned = objData.workout;
        let objGained = objData.student_meal;

        let tempIndex = 0;

        $.each(objBurned, function (k, v) {
            tempIndex = origDate.indexOf(v.workout_date);
            varBurned[tempIndex] = v.title;
            console.log(tempIndex + ':' + v.title);
        });
        $.each(objGained, function (k, v) {
            tempIndex = origDate.indexOf(v.meal_date);
            varGained[tempIndex] = v.title;
        });
        ...
        console.log(err);
        fireSwal('Dashboard', 'Failed to retrieve information. Please reload the page', 'error');
    })
...
$.fn.popover.Constructor.prototype.leave = function (e) {
    ...
    init_sidebar(), init_wysiwyg(), init_InputMask(), init_TagsInput(), init_parsley(), init_daterangepicker(), init_daterangepicker_right(), init_daterangepicker_single_call(), init_daterangepicker_reservation(), init_SmartWizard(), init_select2(), init_validator(), init_PNotify(), init_starrr(), init_compose(), init_autosize(), init_autocomplete()
});